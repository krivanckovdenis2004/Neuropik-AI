...
function App(){
 ...
 return <>
  ...
  <main>
   ...
   {!authLoading&&page==='chat'&&<Chat t={t} user={user} setPage={setPage}/>} 
...
function Chat({t,user,setPage}){const[msg,setMsg]=useState('');const[list,setList]=useState([]);if(!user)return <NeedLogin t={t} setPage={setPage}/>;return <section className="panel chat"><h2>{t.chatTitle}</h2><div className="chatbox">{list.map((m,i)=><div key={i} className={m.role}>{m.text}</div>)}</div><div className="inputrow"><input value={msg} onChange={e=>setMsg(e.target.value)} placeholder={t.placeholder}/><button className="cta" onClick={()=>{if(!msg.trim())return;setList([...list,{role:'me',text:msg},{role:'ai',text:t.demo}]);setMsg('')}}><Send size={18}/>{t.send}</button></div></section>}

function ImageGen({t,user,setPage,credits,dec,addHistory}){const[p,setP]=useState('');const[img,setImg]=useState('');const[busy,setBusy]=useState(false);if(!user)return <NeedLogin t={t} setPage={setPage}/>;async function generate(){if(!p.trim()||credits<1||busy)return;setBusy(true);const ok=await dec();if(!ok){setBusy(false);return;}const svg=`data:image/svg+xml,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='1024' height='1024'><defs><radialGradient id='g'><stop stop-color='#55ff12'/><stop offset='1' stop-color='#020302'/></radialGradient></defs><rect width='100%' height='100%' fill='#020302'/><circle cx='512' cy='420' r='260' fill='url(#g)' opacity='.5'/><text x='512' y='490' text-anchor='middle' fill='#55ff12' font-size='64' font-family='Arial' font-weight='700'>NeuroPic AI</text><text x='512' y='575' text-anchor='middle' fill='white' font-size='30' font-family='Arial'>${p.replace(/[<>&]/g,'').slice(0,48)}</text></svg>`)}`;setImg(svg);await addHistory({prompt:p,img:svg});setBusy(false);}return <section className="panel"><h2>{t.imgTitle}</h2><div className="credits">{t.credits}: <b>{credits}</b></div><textarea value={p} onChange={e=>setP(e.target.value)} placeholder={t.prompt}/><button className="cta big" disabled={!p.trim()||credits<1||busy} onClick={generate}>{busy?t.loading:t.generate}</button>{credits<1&&<p className="warn">{t.buy}</p>}{img&&<div className="result"><img src={img}/><a className="outline" download="neuropic-demo.svg" href={img}><Download/>Download</a></div>}</section>}

function History({t,history}){return <section className="panel"><h2>{t.history}</h2>{!history.length?<p>{t.empty}</p>:<div className="gallery">{history.map((h,i)=><div className="work" key={h.id||i}><img src={h.img}/><b>{h.prompt}</b><small>{h.date||''}</small></div>)}</div>}</section>}
...
function FAQ({lang}){return <section className="panel"><h2>FAQ</h2><h3>{lang==='ru'?'Это уже подключено к настоящему ИИ?':'Is real AI connected?'}</h3><p>{lang==='ru'?'Сейчас подключены настоящая регистрация Firebase, кредиты и история. Ответы ИИ и реальные картинки подключаются следующим этапом.':'Firebase auth, credits and history are connected. Real AI and images come next.'}</p></section>}
